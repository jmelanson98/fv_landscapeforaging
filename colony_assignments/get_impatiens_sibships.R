...
singleton_barcodes <- impatiens2022_forcolony$barcode_id[!impatiens2022_forcolony$barcode_id %in% sibship2022$barcode_id]
...
###############################################################################
###############################################################################
# Now repeat all of that for 2023 impatiens.....
###############################################################################
###############################################################################
...
for (i in 1:5){
  # read in dyad data from COLONY
  fsDyad2023 = trimws(readLines(paste0("/Users/jenna1/Documents/UBC/bombus_project/fv_landscapeforaging/colony_assignments/Colony2_Linux/impatiens2023_final", i, ".FullSibDyad")))
...
png("figures/manuscript_figures/impatiens2023_familystructure_showmissinglinks.png", width = 2000, height = 2000)
plot(graph_23_withmissing,
     ...
     edge.width = 4,
     vertex.size = 2,
     main = "Impatiens Family Structures in 2023 -- Missing Links in Red")
dev.off()
...
png("figures/manuscript_figures/impatiens2023_familystructure.png", width = 2000, height = 2000)
plot(final_graph,
     vertex.label = V(final_graph)$name,
     vertex.label.cex = 0.6,
     vertex.label.color = "black",
     edge.width = 2,
     vertex.size = 2,
     main = "Impatiens Family Structures in 2023")
dev.off()
...
sibship2023 =  data.frame(
  barcode_id = V(final_graph)$name,
  sibship_id = max(impatiens2022colonies$sibship_id) + comp_info$membership
)

# identify barcodes not in a sibship
singleton_barcodes <- impatiens2023_forcolony$barcode_id[!impatiens2023_forcolony$barcode_id %in% sibship2023$barcode_id]
