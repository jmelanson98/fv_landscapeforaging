...
transformed parameters {
  ...
  for (c in 1:C) {
    ...
    // calculate distances
    dis[start:start+length-1] = sqrt( square(delta_x[colony_id[start]] - trap_pos[start:start+length-1,1]) +
...
model {
  ...
  for (c in 1:C) {
    ...
    vector[length] lambda_ik = -0.5*(dis[start:start+length-1] / rho)^2 + 
