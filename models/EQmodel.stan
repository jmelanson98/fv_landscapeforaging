...
real<lower=0> rho_center; //prior median for length parameter
real<lower=0> rho_sd_log; //prior sd of length parameter, !!on log scale!!
}
...
rho_raw ~ normal(log(rho_center), rho_sd_log);  // log-normal with median = rho_center
mu ~ normal(0, priorCo); 
...
for(k in 1:K){
  for(i in 1:C){
    ...
    lambda[i, k] = -0.5*(dis[i,k]/rho - theta*floral[k])^2 + mu + zeta_scale[i] + eps_scale[k];
